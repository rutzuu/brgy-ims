                ...
                <span class="flex items-center space-x-4">
                  <span class="flex-1 flex space-x-2 truncate">
                    ...
                    <span class="flex flex-col text-gray-500 text-sm truncate">
                      <span class="truncate">{{ transaction.number }}</span>
                      <span class="truncate">{{ transaction.name }}</span>
                      <span class="truncate">{{ transaction.product }}</span>
                      <span
                        >₱<span class="text-gray-900 font-medium">{{ transaction.amount }}</span></span
                      >
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="transaction in transactions" :key="transaction.id" class="bg-white">
                      ...
                      <td class="hidden px-6 py-4 whitespace-nowrap text-sm text-gray-500 md:block">
                        <span :class="[productStyles[transaction.product as keyof typeof productStyles], 'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium capitalize']">
                          {{ transaction.product }}
                        </span>
                      </td>
