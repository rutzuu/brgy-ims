...
import { useRoute, useRouter } from 'vue-router'
  ...
  onMounted(async () => {
    ...
    const residentsResponse = await axios.get('/residents/all')

    products.value = productsResponse.data.data
    residents.value = residentsResponse.data

    const response = await axios.get(`/orders/${route.params.id}`)

    data.product_id = response.data.product.id
    data.resident_id = response.data.resident.id
  }) 


  const submit = async () => {
    if (confirm('Confirm editing the existing order? Please make sure that the information is correct.')) {
      await axios.post(`/orders/${route.params.id}`, data)
      ...
      <form @submit.prevent="submit" class="p-4 sm:p-6 lg:p-8 space-y-8 divide-y divide-gray-200">
        <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
          ...
          <div class="pt-8 space-y-6 sm:pt-10 sm:space-y-5">
            <div>
              <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Order</h3>
              <p class="mt-1 max-w-2xl text-sm text-gray-500">Choose between the available products and the existing residents. Those who are not registered yet should sign up first.</p>
            </div>
            <div class="space-y-6 sm:space-y-5">
              <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="Product" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"> Product </label>
